      ...
      db.all(
        ...
        (err2, withdrawals) => {
          ...
          withdrawals.forEach(w => {
            ...
            const availableCash = cash - withdrawnFromCash;

            if (availableCash >= amount) {
              withdrawnFromCash += amount;
            } else {
              if (availableCash > 0) {
                withdrawnFromCash += availableCash;
                withdrawnFromOnline += amount - availableCash;
              } else {
          ...
          const remaining = cash - withdrawnFromCash;

          res.render('collection', {
            bills: rows,
            withdrawals, // ✅ used in EJS to list withdrawals
            summary: {
              cash,
              online,
              total: cash + online,
