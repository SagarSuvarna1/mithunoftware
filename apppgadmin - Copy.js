...
app.get('/collections', async (req, res) => {
  ...
  try {
    // ✅ Step 1: Fetch today's billing grouped by payment mode
    ...
    // ✅ Step 2: Separate cash and online amount
    let cash = 0, online = 0;
    ...
    // ✅ Step 4: Fetch all withdrawals today
    ...
    // ✅ Step 5: Calculate total handed over
    ...
    const alreadyWithdrawn = parseFloat(sumRes.rows[0]?.sum_handover || 0);

    // ✅ Step 6: Remaining = today's cash - total handed over
...
app.post('/withdraw', async (req, res) => {
  ...
  try {
    // ✅ Step 1: Re-fetch today's billing totals
    ...
    // ✅ Step 3: Calculate already handed over
    const sumRes = await db.query(`
    ...
    // ✅ Step 4: Compute remaining after this new handover
    const remaining = cash - (alreadyWithdrawn + handover);

    // ✅ Step 5: Insert handover entry
    await db.query(`
